...
# DATABASE_URL will be an environment variable in Vercel
# Example format: postgresql://user:password@host:port/database
DATABASE_URL = os.environ.get('DATABASE_URL')
EMAIL_DOMAIN = "gmail.com" # Or your chosen domain
EMAILS_PER_PAGE = 10 
...
def init_db(force_create=False):
    ...
    # For PostgreSQL, table creation is usually handled by migrations in larger projects.
    # For this simple app, we'll create them if they don't exist on app startup.
    # The 'force_create' flag is less relevant here unless you want to drop and recreate.
    ...
    try:
        ...
        # Table for generated email addresses
        cursor.execute('''
        ...
        # Table for received email messages
        cursor.execute('''
            CREATE TABLE IF NOT EXISTS received_emails (
                ...
                subject TEXT,
                body TEXT,
                timestamp TIMESTAMPTZ, /* Original email's timestamp */
                received_at TIMESTAMPTZ DEFAULT CURRENT_TIMESTAMP /* When our server got it */
            )
        ''')
...
def api_get_emails(email_address):
    ...
    try:
        ...
        if total_emails_result:
            total_emails = total_emails_result[0]
...
@app.route('/api/email/<email_id_str>', methods=['DELETE']) # email_id_str to avoid conflict with uuid type
def api_delete_email(email_id_str):
    ...
    status_code = 500
    try:
        # Validate if email_id_str is a valid UUID
        email_id = uuid.UUID(email_id_str) # Convert string to UUID object
        conn = get_db_connection()
        cursor = conn.cursor()
        
        cursor.execute("SELECT 1 FROM received_emails WHERE id = %s", (email_id,))
        email_exists = cursor.fetchone()
        if not email_exists:
            message = "Email not found."
            status_code = 404
        else:
            cursor.execute("DELETE FROM received_emails WHERE id = %s", (email_id,))
            conn.commit()
            ...
            if cursor.rowcount > 0:
                ...
                message = "Email not found or already deleted." # Should be caught by existence check
                status_code = 404
    except ValueError: # If email_id_str is not a valid UUID
...
def api_clear_inbox(email_address):
    ...
    try:
        ...
        if not cursor.fetchone():
            ...
            deleted_count = cursor.rowcount # Get count before commit
            conn.commit()
...
def api_simulate_receive_email():
    ...
    try:
        ...
        if not cursor.fetchone():
            ...
            email_id = uuid.uuid4() # Generate a UUID for the email
            # Use TIMESTAMPTZ for PostgreSQL, ensure your timestamp string is compatible or use NOW()
            timestamp_obj = datetime.datetime.now(datetime.timezone.utc) # Store as timezone-aware UTC

            cursor.execute("""
                INSERT INTO received_emails (id, email_address, sender, subject, body, timestamp)
                VALUES (%s, %s, %s, %s, %s, %s)
            """, (email_id, to_address, sender, subject, body, timestamp_obj))
            conn.commit()
...
# --- Initialize DB when app starts (for Vercel, this might run on each cold start) ---
# It's generally better to handle schema migrations separately in production.
# But for this project, ensuring tables exist on startup is okay.
try:
...
if __name__ == '__main__':
    ...
    # Vercel will use the 'app' instance directly
    if not DATABASE_URL:
